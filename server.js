...
const corsOptions = {
	origin:
		process.env.NODE_ENV === 'production'
			...
			: ['https://ecommerce-2026-nine.vercel.app', 'https://ecommerce-2026-nine.vercel.app/'], // Agrega puertos usados por tu frontend en desarrollo
	credentials: true,
};
...
app.put('/api/cart/:cartId', authenticateToken, [body('quantity').isInt({ min: 1 })], async (req, res) => {
	const errors = validationResult(req);
	if (!errors.isEmpty()) {
		return res.status(400).json({ error: 'Invalid input data', details: errors.array() });
	}
